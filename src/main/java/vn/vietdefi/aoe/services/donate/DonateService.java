...
public class DonateService implements IDonateService {
    ...
    public JsonObject donate(long sender, long star, int service, long target_id, String message) {
        try {
            ...
            String targetName = "";
            if (service == StarConstant.SERVICE_DONATE_GAMER || service == StarConstant.SERVICE_DONATE_CASTER) {
                targetName = response.get("data").getAsJsonObject().get("nick_name").getAsString();
            }
            if ( service==StarConstant.SERVICE_DONATE_MATCH){
                targetName = new StringBuilder("kd").append(response.get("data").getAsJsonObject().get("id").getAsString()).toString();
            }
            if (service==StarConstant.SERVICE_DONATE_LEAGUE ){
                 targetName = new StringBuilder("gd").append(response.get("data").getAsJsonObject().get("id").getAsString()).toString();
            }
            ...
            data.addProperty("target_name", targetName);
    ...
    public JsonObject listAllTopDonate(long page, long recordPerPage) {
        try {
            SQLJavaBridge bridge = HikariClients.instance().defaulSQLJavaBridge();
            long offset = (page - 1) * recordPerPage;
            String query = "SELECT user_id,avatar, SUM(amount) as total_star, nick_name FROM aoe_donate GROUP BY user_id ORDER BY total_star DESC LIMIT ? OFFSET ?";
    ...
    public JsonObject listGamerFavorites(long userId, long page, long recordPerPage) {
        try {
            long offset = (page - 1) * recordPerPage;
            SQLJavaBridge bridge = HikariClients.instance().defaulSQLJavaBridge();
            String query = "SELECT DISTINCT target_id FROM aoe_donate WHERE user_id = ? AND service = ? LIMIT ? OFFSET ?";
