...
public class MatchService implements IMatchService {
    ...
    public JsonObject getOutstandingMatch() {
        try {
            SQLJavaBridge bridge = HikariClients.instance().defaulSQLJavaBridge();
            String query = "SELECT * FROM aoe_match WHERE star_current < star_default AND state = ? ORDER BY star_current DESC LIMIT 1";
            JsonObject data = bridge.queryOne(query, MatchConstant.STATE_VOTING);
            if (data == null) {
    ...
    public JsonObject endMatch(JsonObject json) {
        try {
            ...
            data = data.get("data").getAsJsonObject();
            data.get("detail").getAsJsonObject().add("result", json.get("result").getAsJsonArray());
            updateDb.addProperty("id", matchId);
            ...
            JsonObject response = MatchGamerService.updateResultMatch(matchId, data.get("format").getAsInt(), json.get("result").getAsJsonArray());
            return BaseResponse.createFullMessageResponse(0, "success");
        } catch (Exception e) {
            String stacktrace = ExceptionUtils.getStackTrace(e);
            DebugLogger.error(stacktrace);
